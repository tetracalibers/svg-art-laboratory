<script lang="ts">
  ...
  // --- フィボナッチ数列の配列から、正方形を回り込むように敷き詰める関数 ---
  interface FiboSquareSpiralOption {
    width: number
  }
  const fiboSquareSpiral = <T extends SVGTagName>(
    ctx: SkSVG<T>,
    fibo: number[],
    ...
    for (let i = 1; i < n - 1; i++) {
      ...
      drawRect(ctx, {
        ...
        width: f1 * scale * SGN[(i + 1) % 4],
        height: f1 * scale * SGN[i % 4],
  ...
  // -- フィボナッチ長方形（正方形を回り込むように敷き詰めて黄金長方形を近似） ---
  onMount(() => {
    // --- settings ---
    ...
    fiboSquareSpiral(group, fibo, { width: canvasSize })
